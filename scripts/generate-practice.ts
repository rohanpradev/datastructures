...
function extractClassInfo(content: string): Array<{
	...
	const classRegex = /\/\*\*[\s\S]*?\*\/\s*export\s+class\s+\w+[^{]*\{[\s\S]*?\n\}/g;
	...
	while ((match = classRegex.exec(content)) !== null) {
		...
		const methodRegex = /\/\*\*[\s\S]*?\*\/\s*(?:public|private|protected)?\s*(?:static)?\s*\w+\s*(<[^>]+>)?\s*\([^)]*\)\s*:\s*[^{]+/g;
...
`;
	...
	const typeRegex = /^export\s+(?:type|interface)\s+[\s\S]*?(?=\n(?:export|\/\*\*|$))/gm;
...
async function copyTestFiles() {
	const testDirs = [
		...
		"algorithms/tests",
		"data-structures/stack/problems/tests",
	];
	
	for (const testDir of testDirs) {
		const srcTestPath = join(SRC_DIR, testDir);
		const practiceTestPath = join(PRACTICE_DIR, testDir);
		
		const srcExists = await Bun.file(srcTestPath).exists();
		if (!srcExists) continue;
		
		await mkdir(practiceTestPath, { recursive: true });
		
		const testFiles = readdirSync(srcTestPath);
		for (const file of testFiles) {
			if (file.endsWith(".test.ts")) {
				const srcFile = Bun.file(join(srcTestPath, file));
				const destFile = join(practiceTestPath, file);
				
				// Use Bun.write with BunFile for efficient file copy
				await Bun.write(destFile, srcFile);
				console.log(`✓ Copied test: ${testDir}/${file}`);
			}
		}
	}
}
