...
function buildList(values: number[]): Node<number> | null {
  if (values.length === 0) return null;

  const head = new Node<number>(values[0]);
  let current = head;

  for (let i = 1; i < values.length; i++) {
    current.next = new Node<number>(values[i]);
...
function listToArray(head: Node<number> | null): number[] {
  const result: number[] = [];
  let current = head;
...
describe("addTwoNumbers", () => {
  test("should add two single-digit numbers without carry", () => {
    const l1 = buildList([2]);
    const l2 = buildList([3]);
  ...
  test("should add numbers with same length lists", () => {
    // 342 + 465 = 807
  ...
  test("should add numbers with different length lists", () => {
    // 99 + 1 = 100
  ...
  test("should handle both lists being null", () => {
...
describe("addTwoNumbers", () => {
  test("single digit without carry", () => {
    const l1 = buildList([2]);
    const l2 = buildList([3]);

    expect(listToArray(addTwoNumbers(l1, l2))).toEqual([5]);
  });
  ...
  test("different length lists", () => {
    ...
    expect(listToArray(addTwoNumbers(l1, l2))).toEqual([0, 0, 1]);
  });

  test("large list (100 elements)", () => {
    const l1 = buildList(new Array(100).fill(9));
    const l2 = buildList([1]);

    const result = listToArray(addTwoNumbers(l1, l2));
